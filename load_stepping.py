...
def run(nodes, elements, F_max, free_dofs, Emod, A, n_steps=20):
    ...
    TODO:
      1. track_dof = np.argmax(np.abs(F_max))
      2. u = np.zeros(2*len(nodes))
         load_history = [0.0]
         disp_history = [u.copy()]
      3. fuer step in range(1, n_steps+1):
           F_ext = (step / n_steps) * F_max
           u, res_norms = solve_load_step(nodes, elements, u, F_ext,
                                           free_dofs, Emod, A)
           load_history.append(F_ext[track_dof])
           disp_history.append(u.copy())
      4. load_history, disp_history als np.array zurueckgeben, dazu res_norms
         des letzten Schritts (last_residuals)
    """
    raise NotImplementedError("TODO: Lastschritt-Schleife")
